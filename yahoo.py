...
ax1.set_title('効率的フロンティア  ( '+str(start.year) +" - "+str(end.year)+ " )", fontname='Meiryo')
ax2 =  fig.add_subplot(spec[0,1])
ax2.set_title('接点ポートフォリオ', fontname='Meiryo')
ax3 =  fig.add_subplot(spec[1,1])
ax3.set_title('最小分散ポート',fontname='Meiryo')
ax4 =  fig.add_subplot(spec[0,2])
ax4.set_title('接点ポートフォリオ',fontname='Meiryo')
ax5 =  fig.add_subplot(spec[1,2])
ax5.set_title('最小分散ポートフォリオ',fontname='Meiryo')
fig.patch.set_facecolor('black')

# plot frontier, max sharpe & min Volatility values with a scatterplot
df.plot.scatter(x='Volatility', y='Returns', c='Sharpe Ratio',
              cmap='jet', edgecolors='black',alpha=1, ax=ax1)
ax1.set_xlabel('Volatility(リスク)', fontname='Meiryo')
ax1.set_ylabel('Returns(リターン)', fontname='Meiryo')
